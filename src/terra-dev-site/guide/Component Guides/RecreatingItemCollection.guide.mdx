...
The examples also use [terra-clinical-item-view](/components/terra-clinical-item-view/clinical-item-view/clinical-item-view) for each cell of data which is not required. It is used here to to recreate the deprecated `terra-item-collection` but you may create your own cell layout to meet the requirements of your design..
...
Next we build the children that are going into this `List`. Each child should be dropped in `terra-list`'s `Item` and be set as the display prop in an `ItemView` from `terra-clinical-item-view`.
